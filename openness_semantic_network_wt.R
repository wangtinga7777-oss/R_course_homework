############################################################
# 数据目录：D:/R/R_course/fuxian
# 主要输入文件：
#   1. FINAL fluency.csv：动物语义流畅性任务的原始回答数据
#   2. FINAL open.csv：开放性相关问卷和潜变量数据
#
# 主要输出目录：
...
# 为什么需要这个版本：
# 作者公开代码使用的是旧版函数名，例如
# semnetcleaner(), autoDeStr(), cosine(), partboot(), partboot.test()。
# 在当前 SemNetCleaner / SemNeT 版本中，部分函数已经改名、移动或被替代；
# 因此本脚本改用当前 API：
#   textcleaner(), similarity(), bootSemNeT(), test.bootSemNeT() 等。
############################################################
...
# 清空当前 R 环境，避免旧对象残留影响本次复现。
rm(list = ls())

# 指定项目主目录；后续读写文件都基于这个目录。
base_dir <- "D:/R/R_course/fuxian"
# 把工作目录切换到项目主目录。
setwd(base_dir)

# 指定输出目录，用来保存清洗结果、统计结果、网络矩阵和会话信息。
out_dir <- file.path(base_dir, "outputs")
# 如果输出目录不存在，就自动创建。
if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE)

# 设置随机种子，保证涉及随机抽样或 bootstrap 的结果尽量可复现。
set.seed(1234)

# 是否运行 bootstrap。调试时先设为 FALSE；正式复现时再设为 TRUE。
RUN_BOOTSTRAP <- TRUE

# 是否运行敏感性分析。调试时先设为 FALSE；主流程跑通后再设为 TRUE。
RUN_SENSITIVITY <- TRUE

# bootstrap 重复次数。论文使用 1000；测试阶段可临时改成 50 或 100。
N_BOOT <- 1000

# 作者原代码使用 4 个核心。后面会根据本机可用核心数自动限制，
# 避免请求超过本机实际可用核心数。
N_CORES <- 4

# 尽量贴近作者原代码：50% 节点保留条件下使用 weighted = TRUE。
...
# 加载 psych：用于计算量表信度，例如 Cronbach alpha。
install_from_cran_or_runiverse("psych")
# 加载 SemNetCleaner：用于清洗语义流畅性文本并生成二值矩阵。
install_from_cran_or_runiverse("SemNetCleaner")
# 加载 SemNeT：用于语义网络构建、网络指标计算和 bootstrap 检验。
install_from_cran_or_runiverse("SemNeT")

# 当前 API 路线并不严格依赖 NetworkToolbox，
# 但为了兼容作者原始基于 TMFG 的工作流，仍然加载该包。
...
# 将输入对象转换为数值型二值数据框；非数值或缺失值统一处理为 0。
...
# 当前 SemNeT 使用 similarity(data, method = "cosine") 计算余弦相似性。
# 作者旧代码使用 cosine(data, addConstant = .01)。下面这个辅助函数
# 先用当前 API 计算余弦相似性，再补加同样的小常数。
# 计算二值回答矩阵中词条之间的余弦相似性，并可额外加一个小常数。
...
# 计算语义网络指标；同时兼容不同版本 semnetmeas() 的参数写法。
...
# 从 equate() 返回的 list 中提取第 i 个等化后的二值矩阵。
extract_equated <- function(eq, i) {
  # equate() 会返回一个 list；不同版本或不同对象名下，list 元素名称可能不同。
...
# 安全地计算量表信度；如果题项无法转成数值，则返回 NA 而不是直接崩溃。
...
# 作者公开代码把 raw 的第 1 列作为被试 ID。
# 有些 CSV 的第一列列名为空或不规范，如果按列名索引可能得到 NULL；
# 因此这里改用按位置索引 raw[[1]]，提高稳健性。
# 从语义流畅性数据的第 1 列提取被试 ID。
raw_id <- raw[[1]]
...
# 只有当两个文件的 ID 列都有效且能清楚匹配时，才重排 latent 文件。
# 如果 ID 不能可靠匹配，则保留原始行顺序；这与作者原代码按行合并的做法一致。
...
############################
# 5. 使用当前 SemNetCleaner API 清洗语义流畅性数据
############################

# 当前 textcleaner() 更适合把被试 ID 放在行名中，而不是作为数据列传入。
# 因此这里去掉第 1 列 ID，只保留语义流畅性回答列。
# 删除 ID 列，只保留动物回答列作为 textcleaner() 的输入。
...
# 本任务是动物语义流畅性任务，因此优先使用 animals 字典。
# 如果本地安装环境中 animals 字典加载失败，则自动退回到 dictionary = NULL。
# 优先用 animals 字典清洗；如果失败，则不使用字典重新清洗。
cleaned <- tryCatch(
...
# 提取或生成二值矩阵：行是被试，列是动物词条，单元格为 0/1。
...
# 把二值矩阵统一转成数值型数据框，作为后续手工修正和网络分析的基础。
...
# 6.1 如果存在 catefrog，则拆分为 cat 和 frog，并删除 catefrog 原列。
# 检查是否存在作者原代码提到的 catefrog 混合词。
...
# 将修正后的完整二值矩阵命名为 finalFull，供后续分析使用。
finalFull <- con
...
############################
# 7. 合并开放性潜变量与二值回答矩阵，并划分高低组
############################

# 再次检查二值矩阵与开放性数据是否逐行对应。
...
# 把开放性潜变量、被试 ID 和动物二值矩阵合并成一个总数据框。
comb <- data.frame(
...
# 找出高开放性组中至少被提到一次的动物词条。
onlyH <- deHigh[, colSums(deHigh) >= 1, drop = FALSE]
# 找出低开放性组中至少被提到一次的动物词条。
...
# 对低开放性组数据做网络分析前处理：只保留达到最小出现次数的词条。
finLow  <- SemNeT::finalize(deLow, minCase = 2)
# 对高开放性组数据做网络分析前处理：只保留达到最小出现次数的词条。
finHigh <- SemNeT::finalize(deHigh, minCase = 2)

# 等化两组词条集合，保证后续网络比较基于相同节点。
...
# 基于低开放性组相似性矩阵构建 TMFG 网络。
netLow  <- get_tmfg_adjacency(cosLow)
# 基于高开放性组相似性矩阵构建 TMFG 网络。
...
# 如果打开 RUN_SENSITIVITY，则运行敏感性分析，用于检验结果是否依赖特定处理方式。
if (RUN_SENSITIVITY) {
  sensitivity_results <- list()
  
  # 敏感性分析版本：保留低开放性组中至少出现一次的词条。
  lowOne  <- deLow[,  colSums(deLow)  >= 1, drop = FALSE]
  # 敏感性分析版本：保留高开放性组中至少出现一次的词条。
...
############################
# 11. 使用当前 SemNeT API 进行部分网络 bootstrap
############################

# 如果打开 RUN_BOOTSTRAP，则运行部分网络 bootstrap，用于检验网络指标差异的稳定性。
if (RUN_BOOTSTRAP) {
  
  # 定义一个函数：给定节点保留比例 prop，运行一次 bootSemNeT()。
  ...
  # 分别在 50%、60%、70%、80%、90% 节点保留比例下进行 bootstrap。
  nodedrop <- list(
  ...
  # 不同版本的 plot.bootSemNeT 输出格式可能不同，因此用 try() 包裹，避免绘图失败中断整个脚本。
  plot_obj <- try(
    plot(
...
############################
# 12. 导出 Cytoscape 可用的网络文件
############################

# 将高开放性组网络转换为 Cytoscape 可读取的边表格式。
highCyto <- SemNeT::convert2cytoscape(netHigh)
# 将低开放性组网络转换为 Cytoscape 可读取的边表格式。
